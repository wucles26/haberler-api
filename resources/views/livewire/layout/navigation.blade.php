...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ $currentTenant ? route('tenant.dashboard', $currentTenant) : route('dashboard') }}" wire:navigate>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @if ($currentTenant)
                        <x-nav-link :href="route('tenant.dashboard', $currentTenant)" :active="request()->routeIs('tenant.dashboard')" wire:navigate>
                            {{ __('Dashboard') }}
                        </x-nav-link>

                        @can('manageMembers', $currentTenant)
                            <x-nav-link :href="route('tenant.members', $currentTenant)" :active="request()->routeIs('tenant.members')" wire:navigate>
                                {{ __('Members') }}
                            </x-nav-link>
                        @endcan
                        ...
                        @can('manageArticles', $currentTenant)
                            <x-nav-link :href="route('tenant.articles', $currentTenant)" :active="request()->routeIs('tenant.articles')" wire:navigate>
                                {{ __('Articles') }}
                            </x-nav-link>
                        @endcan
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if ($currentTenant)
                <x-responsive-nav-link :href="route('tenant.dashboard', $currentTenant)" :active="request()->routeIs('tenant.dashboard')" wire:navigate>
                    {{ __('Dashboard') }}
